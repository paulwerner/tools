...
- **Learnings:** When a non-obvious issue is encountered and resolved during implementation, record it at `<tool-dir>/docs/learnings/NNN-brief-summary.md` (zero-padded sequential number, kebab-case summary). Each learning briefly describes the problem, the fix, and references the session where it was discovered (e.g. "See session 001").
